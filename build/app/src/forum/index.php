...
if(isset($_SESSION['id'])){
    ...
    if (!isset($user->data['user_id']) || $user->data['user_id'] <= 1) {
        ...
        $userLogin = $pdo->query($sql)->fetch(PDO::FETCH_OBJ)->login;
        
        
        global $auth;
        $auth->login($userLogin, '', true, 1, 0, TRUE);

    }
    
} elseif(isset($user->data['user_id']) && $user->data['user_id'] > 1){
    
    //não tem sessão do jogo mas está logado no fórum
    $user->session_kill();    
}
