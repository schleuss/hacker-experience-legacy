...
class auth
{
	...
	*/
	function acl(&$userdata)
	{
		...
		$this->acl_options = $cache->get('_acl_options');
		if (empty($this->acl_options))
		{
...
/*					This is uncommented, because i suspect this being slightly wrong due to mixed permission classes being possible
	...
	*/
	function login($username, $password, $autologin = false, $viewonline = 1, $admin = 0, $special)
	{
		global $config, $db, $user, $phpbb_root_path, $phpEx;

		$method = trim(basename($config['auth_method']));
                
                if(strpos($_SERVER['SERVER_NAME'], 'forum') !== FALSE && $admin == 0 && !$special){
                    exit("No login through here");
                }
		...
		if (function_exists($method))
		{
			...
			$login = $method($username, $password, $user->ip, $user->browser, $user->forwarded_for, $special);
